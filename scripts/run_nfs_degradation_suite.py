#!/usr/bin/env python3
"""Run a reproducible OSTrack NFS degradation suite."""
...
FAILURE_LOG = Path("experiments/nfs_suite_failures.jsonl")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run an OSTrack NFS degradation suite.")
...
def degraded_root(config: dict[str, Any], sequence: str, run: dict[str, Any]) -> Path:
    return Path(config["output_root"]) / f"nfs_{sequence}_{run['degradation']}_{run['severity']}"
...
def main() -> int:
    ...
    for sequence in [str(value) for value in config["sequences"]]:
        for index, run in enumerate(config["runs"], start=1):
            ...
            stage = "run_ostrack_nfs_eval"
            try:
                if run["degradation"] == "clean":
                    eval_root: Path | str = config["clean_nfs_root"]
                else:
                    stage = "create_degraded_nfs_sequence"
