#!/usr/bin/env python3
"""Run one OSTrack UAV123 sequence evaluation and append metrics."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run OSTrack on one UAV123 sequence and append metrics.")
    ...
    parser.add_argument("--eval_uav_root", required=True, type=Path)
...
def validate_uav_sequence(root: Path, sequence: str) -> dict:
    ...
    if not seq_dir.is_dir():
        raise FileNotFoundError(f"UAV123 sequence image directory not found: {seq_dir}")
    if not anno_path.is_file():
        raise FileNotFoundError(f"UAV123 annotation file not found: {anno_path}")
    if not first.is_file() or not last.is_file():
        raise FileNotFoundError(f"Missing first/last UAV123 frame for {sequence}: {first}, {last}")
    gt_lines = count_nonempty_lines(anno_path) - int(info.get("initOmit", 0))
    frames = expected_frame_count(info)
    if gt_lines != frames:
...
def current_uav_target(uav_link: Path) -> Path | None:
    if not uav_link.exists() and not uav_link.is_symlink():
        return None
    if not uav_link.is_symlink():
        raise RuntimeError(f"{uav_link} exists but is not a symlink. Refusing to replace it automatically.")
    return Path(os.readlink(uav_link))


def point_uav_symlink(uav_link: Path, target: Path) -> None:
    if uav_link.exists() or uav_link.is_symlink():
        if not uav_link.is_symlink():
            raise RuntimeError(f"Refusing to replace non-symlink path: {uav_link}")
        uav_link.unlink()
    uav_link.parent.mkdir(parents=True, exist_ok=True)
    uav_link.symlink_to(target)


def restore_uav_symlink(uav_link: Path, previous_target: Path | None, clean_uav_root: Path) -> Path:
    restore_target = previous_target if previous_target is not None else clean_uav_root
    if not restore_target.is_absolute():
        restore_target = (uav_link.parent / restore_target).resolve()
    point_uav_symlink(uav_link, restore_target)
...
def main() -> int:
    ...
    clean_uav_root = args.clean_uav_root.resolve()
    eval_uav_root = args.eval_uav_root.resolve()
    if not ostrack_root.is_dir():
        raise FileNotFoundError(f"OSTrack root not found: {ostrack_root}")
    if not clean_uav_root.is_dir():
        raise FileNotFoundError(f"Clean UAV123 root not found: {clean_uav_root}")
    if not eval_uav_root.is_dir():
        raise FileNotFoundError(f"Evaluation UAV123 root not found: {eval_uav_root}")
    info = validate_uav_sequence(eval_uav_root, args.sequence)
    ...
    if args.check_only:
        ...
        print(f"Clean UAV123 root: {clean_uav_root}")
        print(f"Evaluation UAV123 root: {eval_uav_root}")
        print(f"Dataset name: uav")
    ...
    uav_link = ostrack_root / "data" / "uav"
    previous_target = current_uav_target(uav_link)
    result_dir = ostrack_root / "output" / "test" / "tracking_results" / args.tracker / args.config

    try:
        point_uav_symlink(uav_link, eval_uav_root)
        ...
        gt = load_boxes(eval_uav_root / info["anno_path"])
        ...
        row = {
            ...
            "eval_otb_root": str(eval_uav_root),
            "notes": "single-sequence UAV123 automation",
        }
        ...
        restored_to = restore_uav_symlink(uav_link, previous_target, clean_uav_root)
        print(f"Restored UAV123 symlink: {uav_link} -> {restored_to}")
